...
%load('TrainedGLMAll.mat')
%predictor = glmval(B,features,'identity');


load('BestEnsemble')
eachNetworkGuess = zeros(length(networksString),1);
for oo = 1:length(networksString)
    clearvars net
    load(strcat(networksString{oo}))
    predictor = feed_forward(net,features');
    if predictor(1) > predictor(2)
        tempClassify = 1;
    else
        tempClassify = -1;
    end
    eachNetworkGuess(oo) = tempClassify;
end
finalGuess = sum(eachNetworkGuess);
if finalGuess>0
    classifyResult = 1;
else
    classifyResult = -1;
