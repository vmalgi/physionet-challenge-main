...
features  = cjy_cwt_feature(assigned_states,PCG_resampled);
features = bsxfun(@minus,features,min_ref);
features = bsxfun(@rdivide,features,nnrange);
features = [features -ones(size(features,1))]*opttheta{1};
%% Running classifyFromHsIntervals.m to obtain the final classification result for the current recording
classifyResult = cjy_svm_classifier(features(index),svmmodel);
