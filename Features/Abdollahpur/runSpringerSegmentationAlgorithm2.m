...
function [assigned_states,denoised_signal] = runSpringerSegmentationAlgorithm2(audio_data, Fs, B_matrix, pi_vector, total_observation_distribution, figures)
...
[PCG_Features, featuresFs] = getSpringerPCGFeatures2(audio_data, Fs,false);

%% Get PCG heart rate

[heartRate, systolicTimeInterval] = getHeartRateSchmidt(audio_data, Fs,false);

[~, ~, qt] = viterbiDecodePCG_Springer(PCG_Features, pi_vector, B_matrix, total_observation_distribution, heartRate, systolicTimeInterval, featuresFs,false);

assigned_states = expand_qt(qt, featuresFs, Fs, length(audio_data));

%% 25-400Hz 4th order Butterworth band pass
denoised_signal = butterworth_low_pass_filter(audio_data,2,400,Fs, false);
denoised_signal = butterworth_high_pass_filter(denoised_signal,2,25,Fs,false);

%% Spike removal from the original paper:
denoised_signal = schmidt_spike_removal(denoised_signal,Fs);

if(figures)
   figure('Name','Derived state sequence');
   t1 = (1:length(denoised_signal))./Fs;
   plot(t1,normalise_signal(denoised_signal),'k');
   hold on;
