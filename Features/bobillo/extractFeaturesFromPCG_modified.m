function F = extractFeaturesFromPCG_modified(assigned_states,PCG)
%% Inputs
% assigned_states: the array of state values assigned to the sound recording.
% PCG: resampled sound recording at 1000 Hz.
%
%% Output
% F: matrix with size #cycles x #features (36 features per segment)
%
%% Start with first complete cycle
n=length(assigned_states);
fnzs=find(abs(PCG)>1E-5, 1 ); % skip leading "zeros" signal
assigned_states=assigned_states(fnzs+1:end);
PCG=PCG(fnzs+1:end);
indx = find(abs(diff(assigned_states))>0)+1;
indx=[1;indx;n-fnzs];
% start at first S1 cycle
f1=find(assigned_states==1, 1 ); % first complete cycle
fcy1=find(indx==f1, 1 );
indx=indx(fcy1:end);
